class_name HyperliminalCatBreeds
## The 20 Hyperliminal (Catsino) cat-breed disguises.
## Each canon race maps 1:1 to a breed that is ONLY visible during the PvE
## portion of PVXC inside the Hyperliminal. Everywhere else the character
## shows the Periliminal race name from RaceDataCharacter.

const BREEDS: Array[Dictionary] = [
	{id="tabby", name="Tabby", pow=0, res=0, spd=0, lck=2, sty=0, texture_type="morphic", primary_color=Color(0.55, 0.42, 0.25), lore="The most common and adaptable of all cat breeds. Luck is their greatest asset."},
...
## Order-matched mapping from RaceDataCharacter.RACES index to cat breed.
static func breed_for_index(index: int) -> Dictionary:
	if index < 0 or index >= BREEDS.size():
		return BREEDS[0]
	return BREEDS[index]

static func breed_for_race_id(race_id: String) -> Dictionary:
	var race := RaceDataCharacter.get_race(race_id)
	if race.is_empty():
		return BREEDS[0]
	return breed_for_index(RaceDataCharacter.RACES.find(race))

static func get_breed(breed_id: String) -> Dictionary:
	for b in BREEDS:
		if b.id == breed_id:
			return b.duplicate()
	return {}
