-- Fix: the draw phase of play_poker (004_poker.sql) ignored held cards
-- entirely and evaluated a brand-new random 5-card hand for scoring, while
-- the frontend (PokerGame.tsx) cosmetically overlaid the held cards on top
-- of whatever the server returned. Result: the displayed final hand and the
-- hand the payout was computed from could be completely different hands.
--
-- Fix: client now sends the actual card values for held positions
-- (p_held_cards, aligned with p_held_indices). The draw phase reconstructs
-- the real final hand by keeping those cards and drawing replacements for
-- the rest from a fresh shuffled deck, excluding cards already held so no
...
create or replace function play_poker(
  ...
  p_phase text,
  p_held_cards integer[] default null
) returns jsonb
...
begin
  ...
  -- draw phase: keep held cards, draw fresh replacements for the rest
  -- (excluding cards already held, so no duplicates can appear)
  v_hand := array[null, null, null, null, null]::integer[];
  if p_held_indices is not null then
    for v_i in 1..array_length(p_held_indices, 1) loop
      v_held_pos := p_held_indices[v_i];
      if v_held_pos >= 0 and v_held_pos <= 4 and p_held_cards is not null and v_i <= array_length(p_held_cards, 1) then
        v_hand[v_held_pos + 1] := p_held_cards[v_i];
      end if;
    end loop;
  end if;

  v_deck := array(select d from unnest(v_deck) d where d <> all(coalesce(array_remove(v_hand, null), array[]::integer[])));

  v_draw_idx := 1;
  for v_i in 1..5 loop
    if v_hand[v_i] is null then
      v_hand[v_i] := v_deck[v_draw_idx];
      v_draw_idx := v_draw_idx + 1;
    end if;
  end loop;
